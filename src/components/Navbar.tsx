...
export default memo(function Navbar() {
  ...
  const handleScroll = useCallback(() => {
    ...
    const sections = ['inicio', 'sobre-mi', 'habilidades', 'certificaciones', 'proyectos', 'ctf', 'contacto'];
    let current = '';
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center">
          ...
          <div className="hidden lg:flex items-center space-x-1">
            ...
            <NavLink href="#sobre-mi" isActive={activeSection === '#sobre-mi'}>
              <User className="w-4 h-4" />
              <span>Sobre Mí</span>
            </NavLink>
            <NavLink href="#habilidades" isActive={activeSection === '#habilidades'}>
              <Shield className="w-4 h-4" />
              <span>Habilidades</span>
            </NavLink>
            <NavLink href="#certificaciones" isActive={activeSection === '#certificaciones'}>
        ...
        >
          <div className="glass-effect rounded-xl p-4">
            <div className="space-y-2">
              {[
                { href: '#inicio', icon: <Home className="w-4 h-4" />, label: 'Inicio' },
                { href: '#sobre-mi', icon: <User className="w-4 h-4" />, label: 'Sobre Mí' },
                { href: '#habilidades', icon: <Shield className="w-4 h-4" />, label: 'Habilidades' },
                { href: '#certificaciones', icon: <Award className="w-4 h-4" />, label: 'Certificaciones' },
                { href: '#proyectos', icon: <Briefcase className="w-4 h-4" />, label: 'Proyectos' },
                { href: '#ctf', icon: <Flag className="w-4 h-4" />, label: 'CTF' },
                { href: '#contacto', icon: <MessageSquare className="w-4 h-4" />, label: 'Contacto' },
              ].map((item) => (
                <NavLink
                  key={item.href}
                  href={item.href}
